...
namespace Arm64Gen {
    ...
    void ARM64XEmitter::SetCodePtrUnsafe(u8 *ptr) { m_code = ptr; }
    ...
    const u8 *ARM64XEmitter::GetCodePtr() const { return m_code; }

    u8 *ARM64XEmitter::GetWritableCodePtr() { return m_code; }
    ...
    void ARM64XEmitter::EncodeCompareBranchInst(u32 op, ARM64Reg Rt,
        ...
        ASSERT_MSG(!(distance & 0x3),
            "%s: distance must be a multiple of 4: %" PRIx64, __func__,
            distance);
    ...
    void ARM64XEmitter::EncodeExceptionInst(u32 instenc, u32 imm) {
        ASSERT_MSG(!(imm & ~0xFFFF),
            "%s: Exception instruction too large immediate: %d", __func__,
            imm);
    ...
    void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, u32 op2, ARM64Reg Rt,
        ...
        ASSERT_MSG(!(imm < -256 || imm > 255), "%s: offset too large %d", __func__,
            imm);
    ...
    void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, ARM64Reg Rt, ARM64Reg Rn,
        ...
        ASSERT_MSG(imm >= 0, "%s(INDEX_UNSIGNED): offset must be positive %d",
            __func__, imm);
        ASSERT_MSG(!(imm & ~0xFFF), "%s(INDEX_UNSIGNED): offset too large %d",
            __func__, imm);
    ...
    void ARM64XEmitter::EncodeLoadStoreUnscaled(u32 size, u32 op, ARM64Reg Rt,
        ARM64Reg Rn, s32 imm) {
        ASSERT_MSG(!(imm < -256 || imm > 255), "%s received too large offset: %d",
            __func__, imm);
        Rt = DecodeReg(Rt);
        Rn = DecodeReg(Rn);

        Write32((size << 30) | (0b111 << 27) | (op << 22) | ((imm & 0x1FF) << 12) |
            (Rn << 5) | Rt);
    }
    ...
    static constexpr u32 MaskImm19(s64 distance) { return distance & 0x7FFFF; }

    static constexpr u32 MaskImm14(s64 distance) { return distance & 0x3FFF; }

    static constexpr u32 MaskImm26(s64 distance) { return distance & 0x3FFFFFF; }
    ...
    void ARM64XEmitter::SetJumpTarget(FixupBranch const &branch) {
        ...
        case 0: // CBZ
        {
            ASSERT_MSG(IsInRangeImm19(distance),
                "%s(%d): Received too large distance: %" PRIx64, __func__,
        ...
        case 3: // TBZ
        {
            ASSERT_MSG(IsInRangeImm14(distance),
                "%s(%d): Received too large distance: %" PRIx64, __func__,
                branch.type, distance);
            ARM64Reg reg = DecodeReg(branch.reg);
        ...
        case 5: // B (uncoditional)
            ASSERT_MSG(IsInRangeImm26(distance),
                "%s(%d): Received too large distance: %" PRIx64, __func__,
                branch.type, distance);
    ...
    void ARM64XEmitter::B(CCFlags cond, const void *ptr) {
        ...
        ASSERT_MSG(IsInRangeImm19(distance),
            "%s: Received too large distance: %p->%p %" PRIi64 " %" PRIx64,
            __func__, m_code, ptr, distance, distance);
    ...
    void ARM64XEmitter::SXTW(ARM64Reg Rd, ARM64Reg Rn) {
        ASSERT_MSG(Is64Bit(Rd), "%s requires 64bit register as destination",
            __func__);
        SBFM(Rd, Rn, 0, 31);
    }
    ...
    void ARM64XEmitter::LDURSW(ARM64Reg Rt, ARM64Reg Rn, s32 imm) {
        ASSERT_MSG(!Is64Bit(Rt), "%s must have a 64bit destination register!",
            __func__);
    ...
    void ARM64XEmitter::ABI_PushRegisters(std::bitset<32> registers) {
        ...
        // everything below SP.
        if (num_regs & 1)
            STR(INDEX_PRE, (ARM64Reg)(X0 + registers[it++]), SP, -stack_size);
        else
            STP(INDEX_PRE, (ARM64Reg)(X0 + registers[it++]),
            (ARM64Reg)(X0 + registers[it++]), SP, -stack_size);

        // Fast store for all other registers, this is always an even number.
        for (int i = 0; i < (num_regs - 1) / 2; i++)
            STP(INDEX_SIGNED, (ARM64Reg)(X0 + registers[it++]),
            (ARM64Reg)(X0 + registers[it++]), SP, 16 * (i + 1));
    ...
    void ARM64XEmitter::ABI_PopRegisters(std::bitset<32> registers) {
        ...
        // least.
        ARM64Reg first = (ARM64Reg)(X0 + registers[it++]);
        ARM64Reg second = (ARM64Reg)0;
        if (!(num_regs & 1))
            second = (ARM64Reg)(X0 + registers[it++]);
        ...
        for (unsigned int i = 0; i < (num_regs - 1) / 2; i++)
            LDP(INDEX_SIGNED, (ARM64Reg)(X0 + registers[it++]),
            (ARM64Reg)(X0 + registers[it++]), SP, 16 * (i + 1));
    ...
    void ARM64FloatEmitter::EmitLoadStoreImmediate(u8 size, u32 opc, IndexType type,
        ...
        if (type == INDEX_UNSIGNED) {
            ...
            ASSERT_MSG(!(imm < -256 || imm > 255),
                "%s immediate offset must be within range of -256 to 256!",
                __func__);
    ...
    void ARM64FloatEmitter::EmitScalar2Source(bool M, bool S, u32 type, u32 opcode,
        ...
        ARM64Reg Rm) {
        ASSERT_MSG(!IsQuad(Rd), "%s only supports double and single registers!",
            __func__);
        Rd = DecodeReg(Rd);
        Rn = DecodeReg(Rn);
        Rm = DecodeReg(Rm);
    ...
    void ARM64FloatEmitter::EmitLoadStoreSingleStructure(bool L, bool R, u32 opcode,
        ...
        Write32((quad << 30) | (0b1101 << 24) | (L << 22) | (R << 21) |
    ...
    void ARM64FloatEmitter::EmitLoadStoreMultipleStructurePost(
        ...
        Write32((quad << 30) | (0b11001 << 23) | (L << 22) | (Rm << 16) |
    ...
    void ARM64FloatEmitter::EncodeLoadStoreRegisterOffset(u32 size, bool load,
        ...
        Write32((encoded_size << 30) | (encoded_op << 22) | (0b111100001 << 21) |
    ...
    void ARM64FloatEmitter::EncodeModImm(bool Q, u8 op, u8 cmode, u8 o2,
        ...
        v.hex = abcdefgh;
        Rd = DecodeReg(Rd);
        Write32((Q << 30) | (op << 29) | (0xF << 24) | (v.in.abc << 16) |
            (cmode << 12) | (o2 << 11) | (1 << 10) | (v.in.defgh << 5) | Rd);
    }
    ...
    void ARM64FloatEmitter::LD1(u8 size, u8 count, ARM64Reg Rt, ARM64Reg Rn) {
        ASSERT_MSG(!(count == 0 || count > 4),
            "%s must have a count of 1 to 4 registers!", __func__);
        u32 opcode = 0;
        if (count == 1)
            opcode = 0b111;
        else if (count == 2)
    ...
    void ARM64FloatEmitter::SMOV(u8 size, ARM64Reg Rd, ARM64Reg Rn, u8 index) {
        bool b64Bit = Is64Bit(Rd);
        ASSERT_MSG(Rd < SP, "%s destination must be a GPR!", __func__);
        ASSERT_MSG(size != 64, "%s doesn't support 64bit destination. Use UMOV!",
            __func__);
        u32 imm5 = 0;
    ...
    void ARM64FloatEmitter::SSHLL(u8 src_size, ARM64Reg Rd, ARM64Reg Rn, u32 shift,
        bool upper) {
        ASSERT_MSG(shift < src_size,
            "%s shift amount must less than the element size!", __func__);
        u32 immh = 0;
    ...
    void ARM64FloatEmitter::FMUL(u8 size, ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
        u8 index) {
        ASSERT_MSG(size == 32 || size == 64, "%s only supports 32bit or 64bit size!",
            __func__);
    ...
    void ARM64FloatEmitter::MOVI(u8 size, ARM64Reg Rd, u64 imm, u8 shift) {
        ...
        if (size == 8) {
            ...
            ASSERT_MSG(!(imm & ~0xFFULL), "%s(size8) only supports 8bit values!",
                __func__);
        }
        else if (size == 16) {
            ASSERT_MSG(shift == 0 || shift == 8,
                "%s(size16) only supports shift of {0, 8}!", __func__);
            ASSERT_MSG(!(imm & ~0xFFULL), "%s(size16) only supports 8bit values!",
                __func__);
            ...
            ASSERT_MSG(!(imm & ~0xFFULL), "%s(size32) only supports 8bit values!",
                __func__);
            switch (shift) {
            case 8:
                cmode |= 2;
        ...
        else // 64
        {
            ...
            for (int i = 0; i < 8; ++i) {
                u8 tmp = (imm >> (i << 3)) & 0xFF;
                ASSERT_MSG(tmp == 0xFF || tmp == 0, "%s(size64) Invalid immediate!",
                    __func__);
                if (tmp == 0xFF)
                    abcdefgh |= (1 << i);
            }
    ...
    void ARM64FloatEmitter::ABI_PushRegisters(std::bitset<32> registers,
        ARM64Reg tmp) {
        ...
        if (bundled_loadstore && tmp != INVALID_REG) {
            ...
            std::vector<ARM64Reg> pair_regs;
            for (auto i = 0; i < registers.count(); ++i) {
                auto it = registers[i];
    ...
    void ARM64FloatEmitter::ABI_PopRegisters(std::bitset<32> registers,
        ARM64Reg tmp) {
        bool bundled_loadstore = false;
        auto num_regs = registers.count();
    ...
    void ARM64XEmitter::AddImmediate(ARM64Reg Rd, ARM64Reg Rn, u64 imm, bool shift,
        bool negative, bool flags) {
    ...
    void ARM64XEmitter::ADDI2R_internal(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
        bool negative, bool flags,
        ARM64Reg scratch) {
        bool has_scratch = scratch != INVALID_REG;
        u64 imm_neg = Is64Bit(Rd) ? ~imm + 1
            : (~imm + 1) & 0xFFFFFFFFuLL;
    ...
    void ARM64XEmitter::ADDSI2R(ARM64Reg Rd, ARM64Reg Rn, u64 imm,
        ARM64Reg scratch) {
